CLASS lhc_ZI_RPP_ORDER DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR zi_rpp_order RESULT result.

    METHODS earlynumbering_cba_Itens FOR NUMBERING
      IMPORTING entities FOR CREATE zi_rpp_order\_Itens.

    METHODS SendOrder FOR MODIFY
      IMPORTING keys FOR ACTION zi_rpp_order~SendOrder.

    METHODS updateFields FOR DETERMINE ON SAVE
      IMPORTING keys FOR zi_rpp_order~updateFields.

ENDCLASS.

CLASS lhc_ZI_RPP_ORDER IMPLEMENTATION.
  ...
  METHOD SendOrder.
    ...
    READ ENTITIES OF zi_rpp_order IN LOCAL MODE
    ENTITY Order
  ...
  METHOD updateFields.
    ...
    LOOP AT lt_data ASSIGNING FIELD-SYMBOL(<lf_data>).
      ...
      MODIFY ENTITIES OF zi_rpp_order IN LOCAL MODE
      ENTITY Order
      UPDATE
